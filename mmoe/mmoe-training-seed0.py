...
_parser = argparse.ArgumentParser(description='MMoE training (seeded)')
_parser.add_argument('--seed', type=int, default=0,
                     help='Random seed for this run (use 0,1,2,3,4 for the paper sweep)')
_parser.add_argument('--gating', type=str, default='multi',
                     choices=['multi', 'shared', 'static', 'uniform'],
                     help='Gating ablation (ISMIR camera-ready, reviewer item 2): '
                          'multi   = full MMoE, separate dynamic gates per task (paper system); '
                          'shared  = ONE dynamic gate g(c_t) used by both tasks '
                          '(tests the "task-specific" claim); '
                          'static  = learned per-task weights alpha_i^k, no dependence on c_t '
                          '(tests the "dynamic" claim); '
                          'uniform = fixed 1/N mixing, no learned weighting (no-op floor).')
_parser.add_argument('--drop-expert', type=str, default='none',
                     choices=['none', 'vocals', 'drums', 'bass', 'other'],
                     help='Optional expert-pool sub-selection ablation: train with '
                          'one expert removed (leave-one-out).')
_parser.add_argument('--deterministic', action='store_true',
                     help='Enable TF deterministic ops (bit-exact reproducibility; '
                          'slower, and errors out if any op lacks a deterministic kernel). '
                          'Not required for the seed sweep — seeding alone is sufficient.')
_args, _ = _parser.parse_known_args()
SEED = _args.seed
GATING = _args.gating
DROP_EXPERT = _args.drop_expert

os.environ['PYTHONHASHSEED'] = str(SEED)
if _args.deterministic:
...
class MMoEMusicModel(tf.keras.Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ── frozen experts ────────────────────────────────────────────────
        _all_experts = [expert_vocals, expert_drums, expert_bass, expert_other]
        _all_names   = ['vocals', 'drums', 'bass', 'other']
        # Optional leave-one-out ablation (--drop-expert): keep_idx selects
        # which of the four stem inputs/experts participate. Default: all.
        self.keep_idx     = [i for i, n in enumerate(_all_names) if n != DROP_EXPERT]
        self.experts      = [_all_experts[i] for i in self.keep_idx]
        self.expert_names = [_all_names[i]   for i in self.keep_idx]
        self.n_experts    = len(self.experts)

        # ── gates (variant selected by --gating, reviewer item 2) ─────────
        # 'multi'   : separate dynamic gate per task            (paper system)
        # 'shared'  : one dynamic gate shared by both tasks     → tests "task-specific"
        # 'static'  : learned per-task alpha, input-independent → tests "dynamic"
        # 'uniform' : fixed 1/N                                 → no-op floor
        self.gating = GATING
        if self.gating == 'multi':
            ...
            self.label_gate    = TaskGate(n_experts=self.n_experts, d_model=n_units,
                                          ...
                                          dropout_rate=gate_dropout,
                                          name='label_gate')
        elif self.gating == 'shared':
            self.shared_gate   = TaskGate(n_experts=self.n_experts, d_model=n_units,
                                          hidden_units=gate_hidden,
                                          dropout_rate=gate_dropout,
                                          name='shared_gate')
        elif self.gating == 'static':
            # One learnable logit vector per task; softmaxed at call time.
            # No gating network, no dependence on the input context c_t.
            self.static_logits_b = tf.Variable(
                tf.zeros([self.n_experts]), trainable=True, name='static_logits_boundary')
            self.static_logits_l = tf.Variable(
                tf.zeros([self.n_experts]), trainable=True, name='static_logits_label')
        # 'uniform' needs no parameters.
    ...
    def call(self, spec, chromagram,
        ...
        stem_inputs = [stem_inputs[i] for i in self.keep_idx]  # leave-one-out
        expert_feats = []
        ...
        # 2-5. Gate + fuse + towers: delegate to call_from_encoded so all
        # gating variants (--gating) share a single code path.
        logits_boun, logits_func, gate_b, gate_l, _ = self.call_from_encoded(
            stacked, valid_len, training=training)
    ...
    def call_from_encoded(self, stacked, valid_len, training=False):
        ...
        # ── expert weighting per --gating variant ────────────────────────
        B = tf.shape(stacked)[0]
        if self.gating == 'multi':
            gate_b = self.boundary_gate(gate_context, training=training)  # [B, T, N]
            gate_l = self.label_gate(gate_context,    training=training)  # [B, T, N]
        elif self.gating == 'shared':
            gate_shared = self.shared_gate(gate_context, training=training)
            gate_b = gate_shared
            gate_l = gate_shared
        elif self.gating == 'static':
            alpha_b = tf.nn.softmax(self.static_logits_b)                 # [N]
            alpha_l = tf.nn.softmax(self.static_logits_l)                 # [N]
            gate_b = tf.tile(alpha_b[tf.newaxis, tf.newaxis, :], [B, T, 1])
            gate_l = tf.tile(alpha_l[tf.newaxis, tf.newaxis, :], [B, T, 1])
        elif self.gating == 'uniform':
            unif   = tf.fill([B, T, self.n_experts], 1.0 / self.n_experts)
            gate_b = unif
            gate_l = unif
    ...
    def train_step(self, data):
        ...
        stems = [stems[i] for i in self.keep_idx]  # leave-one-out (--drop-expert)
    ...
    def test_step(self, data, log_gates=False):
        ...
        # ── Per-track metric logging (multi-seed significance testing) ───────
        # One row per test track with all paper metrics. Collected every test
        # epoch; the training loop resets it before testing and dumps it to CSV
        # whenever a new best model is found, so the saved CSV always matches
        # the reported (best-epoch) numbers.
        if not hasattr(self, 'track_log'):
            self.track_log = []
        sec_np_tracks = sec_ref.numpy()
        for i in range(len(sec_np_tracks)):
            _sid = (sec_np_tracks[i].decode('utf-8')
                    if isinstance(sec_np_tracks[i], bytes) else str(sec_np_tracks[i]))
            self.track_log.append({
                'song':   _sid,
                'F1_05':  float(score_dict['F1_seg'][i]),
                'P_05':   float(score_dict['P_seg'][i]),
                'R_05':   float(score_dict['R_seg'][i]),
                'F1_3':   float(score_dict['F1_seg3'][i]),
                'P_3':    float(score_dict['P_seg3'][i]),
                'R_3':    float(score_dict['R_seg3'][i]),
                'F1_P':   float(score_dict['F1_pair'][i]),
                'P_pair': float(score_dict['P_pair'][i]),
                'R_pair': float(score_dict['R_pair'][i]),
                'ACCa':   float(score_dict['Acc_max'][i]),
                'ACCs':   float(score_dict['Acc_smooth'][i]),
            })
        # ─────────────────────────────────────────────────────────────────────
...
def create_multistem_datasets(config_path, data_base_path):
    """Create train/test datasets with all 4 stems."""
    print("🎯 Creating multi-stem datasets...")
...
def train_mmoe():
    ...
    # 1. Load data
    train_data, test_data = create_multistem_datasets(CONFIG_PATH, DATA_BASE_PATH)
    ...
    tf_train_data = tf_train_data.shuffle(TRAIN_SHUFFLE, seed=SEED,
    ...
    # Output path: the paper system keeps the item-1 sweep path unchanged;
    # ablation variants get their own subtree so nothing collides.
    _variant = GATING if DROP_EXPERT == 'none' else f'{GATING}_drop-{DROP_EXPERT}'
    if _variant == 'multi':
        model_path = f'./mmoe_results_with_rwc/seed_{SEED}'
    else:
        model_path = f'./mmoe_results_with_rwc/ablation_{_variant}/seed_{SEED}'
    ...
    for epoch in range(1, N_EPOCHS + 1):
        ...
        model.track_log = []   # reset per-track log for this test pass
        ...
        if test_F1 > best_test_F1:
            ...
            try:
                ...
                # Per-track metrics of the best epoch → input to significance test
                _df_tracks = pd.DataFrame(model.track_log)
                _df_tracks.insert(0, 'seed', SEED)
                _df_tracks.insert(0, 'system',
                                  'MMoE' if _variant == 'multi' else f'MMoE-{_variant}')
                _df_tracks.to_csv(
                    f'{model_path}/best_models/per_track_metrics_seed{SEED}.csv',
                    index=False)
                print(f'📄 Per-track metrics saved '
                      f'({len(_df_tracks)} tracks) → per_track_metrics_seed{SEED}.csv')
