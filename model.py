...
def load_dataset_config(config_path="/Scratch/repository/msa/MSATSUNGPING/my_dataset_selection_146.json"):
    """Load the dataset selection configuration"""
    with open(config_path, 'r') as f:
        config = json.load(f)
    return config

def create_enhanced_datasets(config_path="/Scratch/repository/msa/MSATSUNGPING/my_dataset_selection_146.json", 
...
#                     vocal_spec = np.load(base_name + '_vocalspec.npy')
#                     vocal_chromagram = np.load(base_name + '_vocalchroma.npy')
...
def load_dataset_files(dataset_name, song_ids, data_path, include_augmented=False):
    """Load files for a specific dataset - ALL 4 STEMS"""
    data = {
        'spec': [],
        'chromagram': [],
        # ═══════════════════════════════════════════════════════════════════
    ...
    for song_id in song_ids:
        try:
            ...
            for spec_file in spec_files:
                ...
                # ═════════════════════════════════════════════════════════════
                # Load ALL required files (mixture + 4 stems)
                # ═════════════════════════════════════════════════════════════
                try:
                    # Mixture features
                    spec = np.load(spec_file)
                    ...
                    # Drums stem
                    drums_spec = np.load(base_name + '_drumspec.npy')
                    drums_chromagram = np.load(base_name + '_drumchroma.npy')
                    
                    # Bass stem
                    bass_spec = np.load(base_name + '_bassspec.npy')
                    bass_chromagram = np.load(base_name + '_basschroma.npy')
                    
                    # Others stem
                    others_spec = np.load(base_name + '_othersspec.npy')
                    others_chromagram = np.load(base_name + '_otherschroma.npy')
                    ...
                    if (chromagram.shape[0] != valid_len or 
                        vocal_spec.shape[0] != valid_len or
                        vocal_chromagram.shape[0] != valid_len or
                        drums_spec.shape[0] != valid_len or
                        drums_chromagram.shape[0] != valid_len or
                        bass_spec.shape[0] != valid_len or
                        bass_chromagram.shape[0] != valid_len or
                        others_spec.shape[0] != valid_len or
                        others_chromagram.shape[0] != valid_len or
                        boundary.shape[0] != valid_len or 
                        function.shape[0] != valid_len):
                        print(f"⚠️ Shape mismatch in {os.path.basename(spec_file)}, skipping")
                        continue
                    ...
                    data['drums_spec'].append(drums_spec)
                    data['drums_chromagram'].append(drums_chromagram)
                    
                    data['bass_spec'].append(bass_spec)
                    data['bass_chromagram'].append(bass_chromagram)
                    
                    data['others_spec'].append(others_spec)
                    data['others_chromagram'].append(others_chromagram)
...
# ═════════════════════════════════════════════════════════════════════════
# UPDATE: create_train_data and create_test_data
# ═════════════════════════════════════════════════════════════════════════

def create_train_data(training_config, dataset_paths):
    """Create training dataset with original + augmented data - ALL 4 STEMS"""
    all_data = {
        'spec': [],
        'chromagram': [],
        'vocal_spec': [],
    ...
    for dataset_name, info in training_config.items():
        ...
        print(f"   ✅ {dataset_name}: {len(dataset_data['spec'])} original + {len(aug_data['spec'])} augmented")
...
def create_test_data(test_config, dataset_paths):
    """Create test dataset with original data only - ALL 4 STEMS"""
    all_data = {
        'spec': [],
        'chromagram': [],
        'vocal_spec': [],
...
def train_v2_cross_modal_only():
    ...
    train_data, test_data = create_enhanced_datasets(
        config_path="/Scratch/repository/msa/MSATSUNGPING/my_dataset_selection_146.json",
    ...
    def create_test_dataset():
        """Helper to create a fresh test dataset"""
        dataset = tf.data.Dataset.from_generator(
            lambda: generator(test_data),
            output_signature=output_signature
        )
        return dataset.padded_batch(TEST_BATCH_SIZE)
    ...
    model_path = './vocals_salami_146_again'
    ...
    test_pred_saver = BestTestPredictionSaver(save_dir=model_path)
    ...
    for epoch in range(1, N_EPOCHS+1):
        ...
        # =====================================================================
        # Testing phase - CREATE FRESH DATASET
        # =====================================================================
        print(color_text("GREEN") + "--testing phase--" + color_text("END"))
        
        tf_test_data = create_test_dataset()  # ✨ Fresh dataset for testing
        
        for i_batch, batch in enumerate(tf_test_data):
            model.test_step(batch)
        
        test_data_size = len(test_data['spec'])
        safe_sample = np.random.randint(min(TEST_BATCH_SIZE, test_data_size)) if test_data_size > 0 else 0
        ...
        if test_F1 > best_test_F1:
            ...
            try:
                ...
                # ✨ CREATE FRESH DATASET for prediction saving
                tf_test_data_for_pred = create_test_dataset()
                
                # Save ALL test predictions
                test_pred_saver.save_test_predictions_simple(
                    model=model,
                    test_dataset=tf_test_data_for_pred,  # ✅ Fresh dataset!
                    test_data=test_data,
                    epoch=epoch,
    ...
    print(f"📂 Test predictions: {model_path}/test_predictions/")
    print("\n" + "="*80)
...
def train_v3_full_model():
    ...
    # =========================================================================
    checkpoint = tf.train.Checkpoint(model=model)
    model_path = './vocals_with_full_model_full_salami'
    os.makedirs(f'{model_path}/all_epochs', exist_ok=True)
    os.makedirs(f'{model_path}/best_models', exist_ok=True)
...
class BestTestPredictionSaver:
    """
    Saves comparison CSV files for ALL test songs
    """
    
    def __init__(self, save_dir):
        self.save_dir = save_dir
        self.predictions_dir = os.path.join(save_dir, 'test_predictions')
    ...
    def save_test_predictions_simple(self, model, test_dataset, test_data, epoch, test_metrics):
        """
        Save comparison CSV for ALL test songs
        """
        print(f"\n🎯 Saving predictions for ALL test songs (epoch {epoch})...")
        ...
        for batch_idx, batch in enumerate(test_dataset):
            ...
            for i in range(batch_size):
                ...
                # Save comparison CSV for this song
        ...
        print(f"✅ Saved {saved_count} comparison CSV files")
        print(f"📂 Location: {self.predictions_dir}")
    ...
    def _save_comparison_csv(self, track_id, interval_ref, label_ref, interval_est, label_est, epoch):
        """
        Save ONLY the comparison CSV for one song
        """
        ...
        comp_file = os.path.join(epoch_dir, f'{safe_track_id}_compare.csv')
        ...
        with open(comp_file, 'w', newline='') as f:
            ...
            max_len = max(len(label_ref), len(label_est))
            for i in range(max_len):
                row = [i + 1]
                
                # Ground truth
                if i < len(label_ref):
                    gt_int = interval_ref[i]
                    gt_label = label_ref[i]
                ...
                # Predicted
                if i < len(label_est):
                    pred_int = interval_est[i]
                    pred_label = label_est[i]
                ...
                # Match
                if (i < len(label_ref) and i < len(label_est)):
                    match = '✓' if label_ref[i] == label_est[i] else '✗'
                    row.append(match)
                else:
                    row.append('')
...
def class_conversion(i, reduced=True):
    """Helper function to convert label ID to name"""
    label_map = {
        0: 'intro',
        1: 'verse',
        2: 'chorus',
    ...
    label_name = label_map.get(i, 'unknown')
    
    if reduced:
        return label_name[0].upper()  # First letter uppercase
    else:
        return label_name.capitalize()  # Full name capitalized
