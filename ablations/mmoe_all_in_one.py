...
# ─────────────────────────────────────────────────────────────────────────────
# All-In-One expert: encode methods (from baselines.py)
# ─────────────────────────────────────────────────────────────────────────────

def _encode_allinone_frozen(self, instruments_mel, valid_len):
    """
    Frozen stage for All-In-One expert.
    instruments_mel: [B, 4, T, 80]
    Runs embeddings + all transformer blocks EXCEPT the last one.
    """
    B = tf.shape(instruments_mel)[0]
    K = tf.shape(instruments_mel)[1]   # 4
    T = tf.shape(instruments_mel)[2]
    F = tf.shape(instruments_mel)[3]   # 80

    x = tf.reshape(instruments_mel, [B * K, T, F])   # [B*4, T, 80]
    x = self.embeddings(x, training=False)             # [B*4, T, 24]

    for block in self.blocks[:-1]:
        x = block(x, training=False)                   # [B*4, T, 24]

    return tf.stop_gradient(x)   # [B*4, T, 24]


def _encode_allinone_last_layer(self, x, instruments_mel, valid_len, training=False):
    """
    Unfrozen last block for All-In-One expert.
    x:               [B*4, T, 24]
    instruments_mel: [B, 4, T, 80]
    Returns:         [B, T, 80]
    """
    ...
    x = self.blocks[-1](x, training=training)            # [B*4, T, 24]
    x = tf.reshape(x, [B, K, T, self.config.dim_embed])  # [B, 4, T, 24]
    x = self.norm(x)                                      # [B, 4, T, 24]
    x = tf.reduce_mean(x, axis=1)                        # [B, T, 24]  pool stems
    x = self.proj_to_80(x)                               # [B, T, 80]
    return x


def load_allinone_expert(checkpoint_path):
    """Load and partially unfreeze the All-In-One expert."""
    print(f"🔄 Loading All-In-One expert from {checkpoint_path} ...")
    config = AllInOneConfig()
    model  = AllInOneModel(config)

    # Build with dummy 4-stem input
    dummy_4stem = tf.zeros((1, 4, 100, 80))
    dummy_len   = tf.constant([100])
    _ = model(dummy_4stem, dummy_len, training=False)

    ckpt   = tf.train.Checkpoint(model=model)
    status = ckpt.restore(checkpoint_path).expect_partial()
    try:
        status.assert_existing_objects_matched()
        print("  ✅ AllInOne: checkpoint restored")
    except Exception as e:
        print(f"  ⚠️  AllInOne: {e}")
    ...
    n_unfrozen = sum(tf.size(v).numpy() for v in last_block.trainable_variables)
    print(f"  🔓 AllInOne: last transformer block unfrozen ({n_unfrozen:,} params)")

    # Add trainable projection Dense(24 → 80) — defined in __init__ of AllInOneModel
    # (proj_to_80 is already defined in the AllInOneModel class)
    model.proj_to_80.trainable = True
    print(f"  ✨ AllInOne: proj_to_80 Dense(24→80) is trainable")

    model.expert_type = 'allinone'
    model._encode_frozen     = types.MethodType(_encode_allinone_frozen,     model)
    model._encode_last_layer = types.MethodType(_encode_allinone_last_layer, model)
    return model
...
class MMoEMusicModel(tf.keras.Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print("\n" + "="*80)
        print("MMoE 5-EXPERT CONFIGURATION")
        print("="*80)
    ...
    def call(self, spec, chromagram,
        ...
        instruments_mel          : [B,4,T,80] stacked stems for All-In-One
        """
        stem_inputs = [
            (vocal_spec,  vocal_chromagram),
            (drum_spec,   drum_chromagram),
        ...
        # Encode All-In-One expert (expert 4)
        alo_exp    = self.experts[4]
        x_frozen   = alo_exp._encode_frozen(instruments_mel, valid_len)
        alo_feat   = alo_exp._encode_last_layer(x_frozen, instruments_mel,
                                                valid_len, training=training)
        expert_feats.append(alo_feat)   # [B, T, 80]
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            # All-In-One last block inside tape
            alo_feat = alo_exp._encode_last_layer(x_frozen, instruments_mel,
                                                   valid_len, training=True)
            enc_list.append(alo_feat)
        ...
        # Split gradients:
        #   SIMI experts  → specTNT_layers[-1]
        #   AllInOne      → blocks[-1] + proj_to_80
        expert_vars = []
        for exp in self.experts[:4]:
            expert_vars.extend(exp.specTNT_layers[-1].trainable_variables)
        # All-In-One expert vars
        expert_vars.extend(self.experts[4].blocks[-1].trainable_variables)
        expert_vars.extend(self.experts[4].proj_to_80.trainable_variables)
...
# ─────────────────────────────────────────────────────────────────────────────
# Data pipeline — exact copy from attention-towers-mmoe.py with instruments_mel
# ─────────────────────────────────────────────────────────────────────────────
...
def make_tf_dataset(data):
    """
    Exact generator from attention-towers-mmoe.py plus instruments_mel
    stack [4,T,80] for All-In-One expert.
    """
    def generator():
        for i in range(len(data['spec'])):
            ...
            # Stack 4 stem specs for All-In-One: [4, T, 80]
            instruments_mel = np.stack([
    ...
    output_signature = (
        ...
        tf.TensorSpec(shape=[4, None, 80], dtype=tf.float32), # instruments_mel
    ...
    def save_predictions(self, model, test_dataset, test_data, epoch):
        ...
        for batch in test_dataset:
            ...
            logits_boun, logits_func, gate_b, gate_l = model.call(
                ...
                instruments_mel, valid_len, training=False
            )
            prob_boun       = tf.nn.sigmoid(logits_boun)
        ...
        if model.gate_log:
            ...
            print(f"  Boundary: V={gate_df['boun_gate_vocals'].mean():.3f}  "
                  ...
                  f"AIO={gate_df['boun_gate_allinone'].mean():.3f}")
            print(f"  Label:    V={gate_df['label_gate_vocals'].mean():.3f}  "
                  ...
                  f"AIO={gate_df['label_gate_allinone'].mean():.3f}")
...
def train_mmoe():
    ...
    expert_allinone = load_allinone_expert(CKPT_ALLINONE)
    ...
    model = MMoEMusicModel(
        ...
        expert_allinone=expert_allinone,
        n_units=80,
        n_classes=7,
        gate_hidden=64,
        gate_dropout=0.1,
    ...
    print("🔄 Building 5-expert MMoE model ...")
    dummy_spec    = tf.zeros((1, 100, 80))
    dummy_chroma  = tf.zeros((1, 100, 12))
    ...
    _ = model(
        ...
        dummy_stem, dummy_stemch,
        dummy_stem, dummy_stemch,
        dummy_4stem, dummy_len, training=False
    )
    ...
    print("🚀 Starting 5-expert MMoE training ...\n")
    ...
    print("\n" + "="*80)
    print("🎉 5-EXPERT MMoE TRAINING COMPLETE!")
