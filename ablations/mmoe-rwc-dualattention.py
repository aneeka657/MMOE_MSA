...
# ─────────────────────────────────────────────────────────────────────────────
# Import Dual-Attention (Tsungping) model class
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Dual-Attention (Tsungping) expert: encode methods (from baselines.py)
# Takes only mixture (spec, chromagram) — no stem inputs
# ─────────────────────────────────────────────────────────────────────────────

def _encode_dualattn_frozen(self, spec, chromagram, valid_len):
    spec       = tf.math.log(1 + 100 * tf.nn.relu(spec + 80))
    spec       = tf.expand_dims(spec,       axis=-1)
    chromagram = tf.expand_dims(chromagram, axis=-1)
    ...
    # Dual-Attention initialises enc_FCT via fct_dense
    enc_FCT = self.fct_dense(tf.reduce_mean(enc_S, axis=2, keepdims=True))
    enc_FCT = self.fct_dense_norm(enc_FCT)
    enc_FCT += self.fpe_FCT

    # Defensive unpacking — SpecTNT_CAMHSA can return 2 or 4 values
    for specTNT in self.specTNT_layers[:-1]:
...
def _encode_dualattn_last_layer(self, enc_S, enc_FCT, valid_len, training=False):
    last_specTNT = self.specTNT_layers[-1]
    # Defensive unpacking
...
def load_tsungping_expert(checkpoint_path):
    """Load and partially unfreeze the Dual-Attention (Tsungping) expert."""
    print(f"🔄 Loading Dual-Attention (Tsungping) expert from {checkpoint_path} ...")
    ...
    try:
        status.assert_existing_objects_matched()
        print("  ✅ Tsungping: checkpoint restored")
    except Exception as e:
        print(f"  ⚠️  Tsungping: {e}")
    ...
    last_spectnt.trainable = True
    n_unfrozen = sum(tf.size(v).numpy() for v in last_spectnt.trainable_variables)
    print(f"  🔓 Tsungping: last SpecTNT_CAMHSA layer unfrozen ({n_unfrozen:,} params)")

    model.expert_type = 'dualattn'
    model._encode_frozen     = types.MethodType(_encode_dualattn_frozen,     model)
    model._encode_last_layer = types.MethodType(_encode_dualattn_last_layer, model)
    return model
...
class MMoEMusicModel(tf.keras.Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.experts      = [expert_vocals, expert_drums, expert_bass,
                             expert_other, expert_tsungping]
        self.expert_names = ['Vocals+Mix', 'Drums+Mix', 'Bass+Mix',
                             'Others+Mix', 'Tsungping']
        self.n_experts    = 5
        ...
        print("\n" + "="*80)
        print("MMoE 5-EXPERT CONFIGURATION (SIMI x4 + Tsungping)")
        print("="*80)
    ...
    def call(self, spec, chromagram,
        ...
        Tsungping expert also uses spec + chromagram (mixture only)
        """
        stem_inputs = [
            (vocal_spec,  vocal_chromagram),
            (drum_spec,   drum_chromagram),
        ...
        # Encode Tsungping (Dual-Attention) expert (expert 4) — mixture only
        tsp_exp             = self.experts[4]
        enc_S, enc_FCT, _, _ = tsp_exp._encode_frozen(spec, chromagram, valid_len)
        tsp_feat            = tsp_exp._encode_last_layer(enc_S, enc_FCT,
                                                         valid_len, training=training)
        expert_feats.append(tsp_feat)   # [B, T, 80]
    ...
    def train_step(self, data):
        ...
        # Tsungping frozen stage (mixture only)
        tsp_exp = self.experts[4]
        tsp_enc_S, tsp_enc_FCT, _, _ = tsp_exp._encode_frozen(spec, chromagram, valid_len)
        ...
        with tf.GradientTape() as tape:
            ...
            # Tsungping last layer inside tape
            tsp_feat = tsp_exp._encode_last_layer(tsp_enc_S, tsp_enc_FCT,
                                                   valid_len, training=True)
            enc_list.append(tsp_feat)
        ...
        # Split gradients:
        #   All 5 experts (SIMI x4 + Tsungping) → specTNT_layers[-1]
        expert_vars = []
...
def make_tf_dataset(data):
    ...
    No instruments_mel needed — Tsungping uses mixture only.
    """
    def generator():
    ...
    def save_predictions(self, model, test_dataset, test_data, epoch):
        ...
        if model.gate_log:
            ...
            print(f"  Boundary: V={gate_df['boun_gate_vocals'].mean():.3f}  "
                  ...
                  f"Tsp={gate_df['boun_gate_tsungping'].mean():.3f}")
            print(f"  Label:    V={gate_df['label_gate_vocals'].mean():.3f}  "
                  ...
                  f"Tsp={gate_df['label_gate_tsungping'].mean():.3f}")
...
def train_mmoe():
    ...
    expert_tsungping = load_tsungping_expert(CKPT_TSUNGPING)
    ...
    model = MMoEMusicModel(
        ...
        expert_tsungping=expert_tsungping,
        n_units=80,
        n_classes=7,
        gate_hidden=64,
        gate_dropout=0.1,
    ...
    print("🔄 Building 5-expert MMoE model (SIMI x4 + Tsungping) ...")
    dummy_spec   = tf.zeros((1, 100, 80))
    dummy_chroma = tf.zeros((1, 100, 12))
    ...
    print("🚀 Starting 5-expert MMoE training (SIMI x4 + Tsungping) ...\n")
    ...
    print("\n" + "="*80)
    print("🎉 5-EXPERT MMoE TRAINING COMPLETE (SIMI x4 + Tsungping)!")
