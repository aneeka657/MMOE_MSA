...
def audio_augmentation(y, sr, method='original'):
    """Apply pitch shift + pre-emphasis"""
    if method == 'original':
        return y
    ...
    if method.startswith('pitch_') and '_preemph_' in method:
        try:
            ...
            if alpha > 0:
                ...
                y_preemph[0] = y_aug[0]
                y_preemph[1:] = y_aug[1:] - alpha * y_aug[:-1]
                y_aug = y_preemph
...
# ============================================================================
# FILENAME MATCHING SYSTEM (CRITICAL!)
# ============================================================================
_filename_cache = {}
...
def create_feature_label(audio_files, annotation_dict, save_dir, downsample, downsample_window, max_len=935):
    """Author's exact create_feature_label function"""
...
def run_salami_preprocessing(
    ...
    audio_dir="/data/beatles_MIX",
    annotation_dir="/data/beatles_MAPPED_ANNOTATIONS",  # ← FIXED!
    output_dir="/data/beatles-Aug-Preprocessed-Data",
    final_dataset_path="./beatles_data/train_data.npz",
    max_len=935
):
    """Complete SALAMI preprocessing"""
    
    print("🎵 Beatles Preprocessing - Author's Method + Filename Matching")
    print("=" * 70)
