...
def create_enhanced_datasets(config_path="/Scratch/repository/msa/MSATSUNGPING/my_dataset_selection_beatles_salami_70_30.json", 
    ...
    os.makedirs('./beatles_salami', exist_ok=True)
        ...
        try:
            ...
            for spec_file in spec_files:
                ...
                try:
                    ...
                    drum_spec = np.load(base_name + '_drumspec.npy')
                    drum_chromagram = np.load(base_name + '_drumchroma.npy')
                    ...
                    if (chromagram.shape[0] != valid_len or 
                        drum_spec.shape[0] != valid_len or
                        drum_chromagram.shape[0] != valid_len or
                    ...
                    data['drum_spec'].append(drum_spec)
                    data['drum_chromagram'].append(drum_chromagram)
...
def train():
    ...
    output_signature = (
        ...
        tf.TensorSpec(shape=[None, 80], dtype=tf.float32),    # stem_spec
        tf.TensorSpec(shape=[None, 12], dtype=tf.float32),    # stem_chromagram
        tf.TensorSpec(shape=[], dtype=tf.int32),              # len (scalar)
        tf.TensorSpec(shape=[None], dtype=tf.int32),          # boundary
        tf.TensorSpec(shape=[None], dtype=tf.int32),          # function class
        tf.TensorSpec(shape=[], dtype=tf.string),             # section (scalar string)
    )
    ...
    # Build the model
    # Build the model
    print("🔧 Building SpecTNT baseline model...")
    dummy_spec = tf.zeros((1, 100, 80))
    dummy_chroma = tf.zeros((1, 100, 12))
    dummy_len = tf.constant([100])
    _ = model(dummy_spec, dummy_chroma, dummy_len, training=False)  # REMOVED drum inputs
    print("✅ SpecTNT baseline model built successfully!")

    # Checkpoint setup
    checkpoint = tf.train.Checkpoint(model=model)
    model_path = './beatles_salami_spectnt_baseline'
    ...
    early_stopping = ImprovedEarlyStopping(patience=25, min_delta=0.001, restore_best=True)
...
class SpecTNT(tf.keras.layers.Layer):
    """Simple SpecTNT without cross-attention"""
    def __init__(
        self,
        n_units=80,
        n_heads_f=4,
    ...
    def call(self, S, FCT, valid_len, training=None):
        # S = [b, n, f, d/2], FCT = [b, n, 1, d]
        b, n, f, half_d = shape_list(S)

        # Spectral attention mask
        ...
        if self.return_maps:
            enc_FCT, map_T = self.attn_t(enc_FCT, valid_len=valid_len, attn_mask=attn_mask_t)
        else:
            enc_FCT = self.attn_t(enc_FCT, valid_len=valid_len, attn_mask=attn_mask_t)
        
        enc_FCT = self.ffn_t(enc_FCT)  # [b, n, d]
        enc_FCT = tf.expand_dims(enc_FCT, axis=2)  # [b, n, 1, d]

        # ✅ CORRECT: Return enc_S and enc_FCT
        if self.return_maps:
            return enc_S, enc_FCT, map_S
        else:
            return enc_S, enc_FCT


class FunctionalSegmentModel(tf.keras.Model):
    """SpecTNT Baseline Model"""
    def __init__(
        self,
        max_len=935,
        n_units=80,
        ...
        **kwargs
        ...
        # CNN layers (keep these - they're the same)
        ...
        # CHANGED: Use simple SpecTNT layers with return_maps=False
        self.specTNT_layers = [SpecTNT(
            n_units=n_units,
            max_len=max_len,
            dropout_rate=attn_dropout_rate,
            return_maps=False,  # ✅ Simplified for baseline
    ...
    def call(self, spec, chromagram, valid_len, training=False):
        '''
        SIMPLIFIED: Only spec and chromagram (no drums)
        spec = [b, n, 80]
        chromagram = [b, n, 12]
        valid_len = [b]
        '''
        ...
        # Pre-Norm
        spec = self.spec_prenorm(spec, valid_len, training=training)

        with tf.name_scope("cnn") as scope_cnn:
            enc_spec = self.specCNNBase(spec, valid_len, training=training)
            enc_spec = self.specCNN(enc_spec, valid_len, training=training)
            ...
            enc_chroma = self.chromaCNNBase(chromagram, valid_len, training=training)
            enc_chroma = self.chromaCNN(enc_chroma, valid_len, training=training)
        ...
        with tf.name_scope("attention") as scope_attn:
            ...
            # Process through SpecTNT layers
        ...
        with tf.name_scope('boundary_estimation') as scope_boun:
            logits_boun = self.boun1(enc)
            logits_boun = self.boun2(logits_boun)
            logits_boun = tf.squeeze(self.boun_out(logits_boun), axis=2)
            logits_boun = logits_boun + enc_spec_res + enc_chroma_res

        with tf.name_scope("function_estimation") as scope_func:
            logits_func = self.func1(enc)
            logits_func = self.func2(logits_func)
            logits_func = self.func_out(logits_func)

        # return logits_boun, logits_func, enc_S, map_S
        return logits_boun, logits_func, enc_S, None  # ✅ None for map_S since return_maps=False

    def train_step(self, data):
        # CHANGED: Only unpack spec and chromagram (no drums)
        spec, chromagram, _, _, valid_len, boun_ref, func_ref, sec_ref = data

        with tf.GradientTape() as tape:
            # CHANGED: Only pass spec and chromagram
    ...
    def test_step(self, data):
        # CHANGED: Only unpack spec and chromagram
        spec, chromagram, _, _, valid_len, boun_ref, func_ref, sec_ref = data
    ...
    def cce_from_logits(self, gt, logits, valid_len):
        ...
        masked_loss = ce * seq_mask
        
        return tf.reduce_mean(tf.reduce_sum(masked_loss, axis=1) / tf.cast(valid_len, tf.float32))
...
if __name__ == '__main__':
    ...
    # Test SpecTNT layer
    spec_tnt = SpecTNT(n_units=80, max_len=935, dropout_rate=0.1, return_maps=False)
    dummy_S = tf.zeros((2, 100, 92, 40))
    dummy_FCT = tf.zeros((2, 100, 1, 80))
    dummy_len = tf.constant([100, 95])
    
    enc_S_out, enc_FCT_out = spec_tnt(dummy_S, dummy_FCT, dummy_len)
    print(f"✅ SpecTNT test passed! Output shapes: {enc_S_out.shape}, {enc_FCT_out.shape}")
    
    # Test full model
    model = FunctionalSegmentModel(max_len=935, n_units=80, n_heads=8, n_layers=2)
    dummy_spec = tf.zeros((2, 100, 80))
    dummy_chroma = tf.zeros((2, 100, 12))
    
    logits_b, logits_f, enc_S, map_S = model(dummy_spec, dummy_chroma, dummy_len, training=False)
    print(f"✅ Model test passed!")
    print(f"   logits_boun: {logits_b.shape}")
    print(f"   logits_func: {logits_f.shape}")
    print(f"   map_S: {map_S}")
