...
# Identical mechanism to the seeded MMoE script: same seed values (0-4).
# ─────────────────────────────────────────────────────────────────────────────
import argparse
import random

_parser = argparse.ArgumentParser(description='All-In-One baseline training (seeded)')
...
def train_tensorflow_allinone():
    ...
    tf_train_data = tf_train_data.shuffle(TRAIN_SHUFFLE_SIZE, seed=SEED, reshuffle_each_iteration=True)
    ...
    model_path = f'./allinone_seed0/seed_{SEED}'
    ...
    for epoch in range(1, N_EPOCHS+1):
        ...
        if test_F1 > sum([float(best_test_result.get(k, 0)) for k in supervised_metrics]):
            ...
            try:
                ...
                _df_tracks = _pd.DataFrame(model.track_log)
                _df_tracks.insert(0, 'seed', SEED)
                _df_tracks.insert(0, 'system', 'All-In-One')
                _df_tracks.to_csv(
                    f'{model_path}/best_models/per_track_metrics_seed{SEED}.csv',
                    index=False)
                print(f'📄 Per-track metrics saved ({len(_df_tracks)} tracks)')
            except Exception as e:
